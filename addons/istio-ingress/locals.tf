locals {
  istio_base = {
    default_helm_config = {
      ...
      chart                      = "base"
      repository                 = try(var.istio_ingress_extra_configs.istiobase_repository, "https://istio-release.storage.googleapis.com/charts")
      version                    = try(var.istio_ingress_extra_configs.istiobase_version, "1.18.0")
  ...
  istiod = {
    default_helm_config = {
      ...
      chart                      = "istiod"
      repository                 = try(var.istio_ingress_extra_configs.istiod_repository, "https://istio-release.storage.googleapis.com/charts")
      version                    = try(var.istio_ingress_extra_configs.istiod_version, "1.18.0")
      namespace                  = try(var.istio_ingress_extra_configs.namespace, "istio-system")
      create_namespace           = try(var.istio_ingress_extra_configs.create_namespace, true)
      description                = "Istio helm Chart deployment configuration"
      timeout                    = try(var.istio_ingress_extra_configs.timeout, "600")
