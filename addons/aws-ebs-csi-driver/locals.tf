locals {
  ...
  default_helm_config = {
    ...
    repository                 = try(var.aws_ebs_csi_driver_extra_configs.repository, "oci://public.ecr.aws/ebs-csi-driver/charts")
    version                    = try(var.aws_ebs_csi_driver_extra_configs.version, "2.63.1")
    ...
    keyring                    = try(var.aws_ebs_csi_driver_extra_configs.keyring, null)
