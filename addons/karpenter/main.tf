...
yaml_body = file(var.karpenter_extra_configs.ec2_nodeclass_yaml)
}
resource "kubectl_manifest" "nodepools" {
  ...
  yaml_body = file(var.karpenter_extra_configs.nodepool_yaml)
}
  ...
  policy      = var.iampolicy_json_content != null ? var.iampolicy_json_content : <<-EOT
{
    "Statement": [
        ...
        },
        {
            "Effect": "Allow",
            "Action": "iam:PassRole",
            "Resource": "${var.karpenter_extra_configs.rolename}",
            "Sid": "PassNodeIAMRole"
        },
        {
            "Effect": "Allow",
